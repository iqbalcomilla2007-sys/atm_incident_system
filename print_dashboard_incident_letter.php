...
$atmVendor = trim((string)($row['atm_vendor_name'] ?? ''));
if ($atmVendor === '') $atmVendor = trim((string)($row['master_atm_vendor'] ?? ''));

$upsVendor = trim((string)($row['ups_vendor_name'] ?? ''));
if ($upsVendor === '') $upsVendor = trim((string)($row['master_ups_vendor'] ?? ''));

$responsibleVendor = '';
$vendorMobile = '';

// 1. First Priority: If problem_master specifies a type like ED, CCTV (Anything other than ATM/CRM/UPS), override everything!
if ($responsibleVendorType !== '' && !in_array($responsibleVendorType, ['ATM', 'CRM', 'UPS'])) {
    $responsibleVendor = $responsibleVendorType; // This will set it to 'ED'
}
// 2. Second Priority: If it's a UPS problem
elseif ($responsibleVendorType === 'UPS') {
    $responsibleVendor = $upsVendor !== '' ? $upsVendor : $savedResponsibleVendor;
}
// 3. Third Priority: Trust the manually saved vendor if priority 1 and 2 didn't trigger
elseif ($savedResponsibleVendor !== '') {
    $responsibleVendor = $savedResponsibleVendor;
}
// 4. Default Fallback: ATM Vendor
else {
    $responsibleVendor = $atmVendor;
}
...
// Fetch Mobile Number & Full Vendor Name from vendor_master / vendor_contacts
// Now searches by both vendor_name AND vendor_type to catch "ED"
if ($responsibleVendor !== '') {
    $stmtMobile = $conn->prepare("
        SELECT vm.vendor_name, vm.vendor_mobile, vc.contact_value 
        FROM vendor_master vm 
        LEFT JOIN vendor_contacts vc ON vc.vendor_id = vm.id AND vc.contact_type = 'mobile' 
        WHERE LOWER(TRIM(vm.vendor_name)) = LOWER(TRIM(?))
           OR LOWER(TRIM(vm.vendor_type)) = LOWER(TRIM(?))
    ");
    if ($stmtMobile) {
        $stmtMobile->bind_param("ss", $responsibleVendor, $responsibleVendor);
        $stmtMobile->execute();
        $mobileRes = $stmtMobile->get_result();
        
        $mobilesArray = [];
        while ($mRow = $mobileRes->fetch_assoc()) {
            if (!empty($mRow['vendor_name'])) {
                // This updates 'ED' to 'Engineering Division'
                $responsibleVendor = trim((string)$mRow['vendor_name']); 
            }
            if (!empty($mRow['vendor_mobile'])) {
                $mobilesArray[] = trim((string)$mRow['vendor_mobile']);
            }
            if (!empty($mRow['contact_value'])) {
                $mobilesArray[] = trim((string)$mRow['contact_value']);
            }
        }
        $stmtMobile->close();
