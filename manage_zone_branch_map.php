...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($zone_name === '' || $branch_name === '') {
        ...
        if ($isDuplicateCode) {
            ...
            // ========================================================
            $new_group_no = null;
            $stmtGrp = $conn->prepare("SELECT group_no FROM group_details WHERE zones LIKE ? LIMIT 1");
            if ($stmtGrp) {
                $likeZone = "%" . $zone_name . "%";
                $stmtGrp->bind_param("s", $likeZone);
                $stmtGrp->execute();
                $resGrp = $stmtGrp->get_result();
                if ($rowGrp = $resGrp->fetch_assoc()) {
                    $new_group_no = (int)$rowGrp['group_no'];
                }
                $stmtGrp->close();
            }
            ...
            if ($id > 0) {
                ...
                if ($new_group_no !== null) {
                    ...
                    $stmtAtm->bind_param("sssis", $branch_code, $zone_name, $branch_name, $new_group_no, $oldBranchTrimmed);
                } else {
                    $stmtAtm = $conn->prepare("UPDATE atm_master SET branch_code = ?, zone_name = ?, branch_name = ? WHERE TRIM(branch_name) = ?");
                    $stmtAtm->bind_param("ssss", $branch_code, $zone_name, $branch_name, $oldBranchTrimmed);
                }
                if ($stmtAtm) { $stmtAtm->execute(); $stmtAtm->close(); }

                // 2. Synchronize with cctv_list (TRIM added for accurate matching)
                $stmtCctv = $conn->prepare("UPDATE cctv_list SET br_code = ?, zone_name = ?, branch_name = ? WHERE TRIM(branch_name) = ?");
                if ($stmtCctv) {
                    $stmtCctv->bind_param("ssss", $branch_code, $zone_name, $branch_name, $oldBranchTrimmed);
                    $stmtCctv->execute();
                    $stmtCctv->close();
                }

                // 3. Synchronize with cctv_locations (with group_no & TRIM)
                if ($new_group_no !== null) {
                    $stmtCctvLoc = $conn->prepare("UPDATE cctv_locations SET zone_name = ?, branch_name = ?, group_no = ? WHERE TRIM(branch_name) = ?");
                    $stmtCctvLoc->bind_param("ssis", $zone_name, $branch_name, $new_group_no, $oldBranchTrimmed);
                } else {
                    $stmtCctvLoc = $conn->prepare("UPDATE cctv_locations SET zone_name = ?, branch_name = ? WHERE TRIM(branch_name) = ?");
                    $stmtCctvLoc->bind_param("sss", $zone_name, $branch_name, $oldBranchTrimmed);
                }
                if ($stmtCctvLoc) { $stmtCctvLoc->execute(); $stmtCctvLoc->close(); }

                // 4. Synchronize with atm_contact
                ...
                if ($stmt->execute()) {
                    ...
                    // 1. Synchronize with atm_master (with group_no)
                    if ($new_group_no !== null) {
                        $stmtAtm = $conn->prepare("UPDATE atm_master SET branch_code = ?, zone_name = ?, group_no = ? WHERE TRIM(branch_name) = ?");
                        $stmtAtm->bind_param("ssis", $branch_code, $zone_name, $new_group_no, $branchTrimmed);
                    } else {
                        $stmtAtm = $conn->prepare("UPDATE atm_master SET branch_code = ?, zone_name = ? WHERE TRIM(branch_name) = ?");
                        $stmtAtm->bind_param("sss", $branch_code, $zone_name, $branchTrimmed);
                    }
                    if ($stmtAtm) { $stmtAtm->execute(); $stmtAtm->close(); }
                    
                    // 2. Synchronize with cctv_list (Added here to fix the issue)
                    $stmtCctv = $conn->prepare("UPDATE cctv_list SET br_code = ?, zone_name = ? WHERE TRIM(branch_name) = ?");
                    if ($stmtCctv) {
                        $stmtCctv->bind_param("sss", $branch_code, $zone_name, $branchTrimmed);
                        $stmtCctv->execute();
                        $stmtCctv->close();
                    }

                    // 3. Synchronize with cctv_locations (with group_no)
                    if ($new_group_no !== null) {
                        $stmtCctvLoc = $conn->prepare("UPDATE cctv_locations SET zone_name = ?, group_no = ? WHERE TRIM(branch_name) = ?");
                        if ($stmtCctvLoc) {
                            $stmtCctvLoc->bind_param("sis", $zone_name, $new_group_no, $branchTrimmed);
                            $stmtCctvLoc->execute();
                            $stmtCctvLoc->close();
                        }
                    } else {
                        $stmtCctvLoc = $conn->prepare("UPDATE cctv_locations SET zone_name = ? WHERE TRIM(branch_name) = ?");
                        if ($stmtCctvLoc) {
                            $stmtCctvLoc->bind_param("ss", $zone_name, $branchTrimmed);
                            $stmtCctvLoc->execute();
                            $stmtCctvLoc->close();
                        }
                    }
