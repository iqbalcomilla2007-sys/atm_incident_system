...
$limit = 20; // প্রতি পেজে ২০টি করে দেখাবে (চাইলে বাড়াতে পারেন)
$offset = ($page - 1) * $limit;
...
$countSql = "SELECT COUNT(*) as total FROM atm_device_movement m LEFT JOIN users u ON m.created_by = u.id $whereClause";
...
$sql = "SELECT m.*, u.username FROM atm_device_movement m LEFT JOIN users u ON m.created_by = u.id $whereClause ORDER BY m.id DESC LIMIT ?, ?";
...
<div class="card">
    ...
    <div class="pagination">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="?page=<?= $i ?>&history_search=<?= urlencode($historySearch) ?>&history_type=<?= urlencode($historyType) ?>" class="<?= ($page == $i) ? 'active' : '' ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
    <?php endif; ?>
</div>
