...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['add_atm']) || isset($_POST['update_atm']))) {
    ...
    if ($result['success']) {
        ...
        // -----------------------------------------------------
        // SYNC LOGIC: Update CCTV Tables when ATM is Updated
        // -----------------------------------------------------
        if (isset($_POST['update_atm'])) {
            ...
            if ($atm_id !== '') {
                // 1. Sync cctv_list table
                $stmtCctvSync = $conn->prepare("
                    UPDATE cctv_list 
                    SET br_code = ?, branch_name = ?, zone_name = ?, atm_name = ? 
                    WHERE atm_id = ?
                ");
                if ($stmtCctvSync) {
                    $stmtCctvSync->bind_param("sssss", $branch_code, $branch_name, $zone_name, $atm_name, $atm_id);
                    $stmtCctvSync->execute();
                    $stmtCctvSync->close();
                }
                
                // 2. Sync cctv_locations table
                $stmtCctvLocSync = $conn->prepare("
                    UPDATE cctv_locations 
                    SET branch_name = ?, booth_name = ?, zone_name = ? 
                    WHERE atm_id = ?
                ");
                if ($stmtCctvLocSync) {
                    $stmtCctvLocSync->bind_param("ssss", $branch_name, $atm_name, $zone_name, $atm_id);
                    $stmtCctvLocSync->execute();
                    $stmtCctvLocSync->close();
                }
            }
