...
// ডেটাবেস থেকে Important Links ফেচ করা হচ্ছে
$nav_links_result = null;
try {
    if (class_exists('Database')) {
        $nav_conn = Database::getInstance()->getConnection();
        $nav_links_result = $nav_conn->query("SELECT link_name, url FROM important_links ORDER BY serial_no ASC");
    } elseif (isset($conn)) {
        $nav_links_result = $conn->query("SELECT link_name, url FROM important_links ORDER BY serial_no ASC");
    }
} catch (Exception $e) {
...
<nav class="global-nav">
    <ul>
        ...
        <li>
            <a href="#" style="color: #60a5fa;">Important Links &#9662;</a>
            <ul class="dropdown-menu">
                <?php 
                if ($nav_links_result && $nav_links_result->num_rows > 0) {
                    while($nav_link = $nav_links_result->fetch_assoc()) { 
                        echo '<li><a href="'.htmlspecialchars($nav_link['url']).'" target="_blank">'.htmlspecialchars($nav_link['link_name']).'</a></li>';
                    }
                } else {
